            ...
            try (BufferedReader reader = new BufferedReader(fileReader)) {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (line.length() != 0 && line.charAt(0) == '@') {
                        ...
                        if (index != -1) {
                            temp_Word.setWord_target(line.substring(1, index));
                            temp_Word.setWord_explain(temp_Word.getWord_explain().concat(line.substring(index, line.length()) + "\r\n"));
                        } else {
                            temp_Word.setWord_target(line.substring(1, line.length()).trim());
                        }
                        while ((line = reader.readLine()) != null && line.length() != 0 && line.charAt(0) != '@') {
                            temp_Word.setWord_explain(temp_Word.getWord_explain().concat(line + "\n"));
                        }
                        dictionary.getWords().add(temp_Word);
                    }
                }
            }
        } catch (IOException e) {
        ...
        try {
            try (FileWriter writer = new FileWriter("dictionaries.txt")) {
                for (int i = 0; i < dictionary.getSizeWords(); i++) {
                    writer.write("@" + dictionary.getWords().get(i).getWord_target() + "\r\n"
